...
Ниже представлен детализированный отчет по автоматизации обновления базы знаний, включающий описание архитектуры, логику работы механизмов защиты (Lock) и описание параметров запуска.

### 9.5. Архитектурная диаграмма (PlantUML)

Эта диаграмма наглядно показывает жизненный цикл данных от момента изменения файла в базе знаний до обновления векторного индекса.

![Диаграмма последовательности](service_c4.png)

[Открыть исходный файл диаграммы (.puml)](diagram.puml)

----------

### 9.6. Описание параметров запуска `update_index.py`

Скрипт поддерживает гибкую настройку через аргументы командной строки, что позволяет использовать его как для ручной отладки, так и для автоматизации.


| Аргумент | Тип | По умолчанию | Описание | Влияние на Lock/Registry |
|---|---|---|---|---|
| --force | флаг | False | Принудительный перезапуск процесса индексации. | Игнорирует реестр, но удерживает Lock до конца всех попыток. |
| --test | флаг | False | Запуск тестов после каждого успешного прохода. | Проверка проводится внутри цикла попыток (Retry Loop). |
| --retries | int | 3 | Макс. число попыток при сбое (сеть, память, API). | Блокировка (Lock) не снимается между попытками. |
| --timeout | int | 3600 | (Внутренний) Лимит времени в секундах на одну итерацию. | При превышении вызывает исключение и переход к следующему Retry. |
----

### 9.7. Механизм эксклюзивного доступа (Locking)

Для предотвращения повреждения индекса при одновременном запуске нескольких экземпляров скрипта (например, если задача в планировщике запустилась до того, как завершился ручной запуск), реализован механизм **File Lock**.

-   **Реализация для Windows**: Используется системный вызов `msvcrt.locking`. Скрипт пытается заблокировать первый байт файла `index.lock`. Если файл уже занят другим процессом, скрипт немедленно завершает работу с сообщением об ошибке, не затрагивая индекс.
    
-   **Реализация для Unix/WSL**: Используется стандарт `fcntl.flock` с флагами `LOCK_EX | LOCK_NB`.
    

----------

### 9.8. Структура логов и хранения

Скрипт автоматически определяет свое местоположение (`BASE_DIR`), что критически важно для работы через планировщик задач Windows, который часто запускает процессы из системных папок.

-   **update.log**: Содержит историю всех запусков, количество обнаруженных изменений и время, затраченное на генерацию эмбеддингов.
    
-   **kb_registry.json**: Хранит карту `имя_файла: дата_изменения`. Это позволяет выполнять "инкрементальную проверку" за доли секунды, не нагружая систему полной пересборкой без необходимости.
    

### Команда для ручной проверки (из папки проекта):

DOS

```
python update_index.py --force --test --retries 5

```

Эта команда принудительно обновит базу, проверит её работу и даст 5 попыток на случай сетевых сбоев при загрузке весов модели.



## 10. Отчет по тестированию RAG-системы и анализу «слепых зон» (Задание 7)

**10.1. Внесение искусственных пробелов в базу** 
Из базы знаний были намеренно исключены три ключевые сущности для симуляции «слепых зон»:

-   Планета **VoidCore**
    
-   Персонаж **Xarn Velgor**
    
-   Концепт **Synth Flux** 

Цель — проверить способность системы корректно признавать отсутствие информации, а не выдумывать факты.
​    

**10.2. Скрипт логирования запросов** Был реализован механизм логирования, который сохраняет каждый прогон в формате JSONL (`evaluation_results.jsonl`). В лог записываются следующие метрики:

-   `timestamp` (время запроса)
    
-   `query` (текст запроса)
    
-   `has_chunks` (найдены ли документы в индексе)
    
-   `sources` (список релевантных источников)
    
-   `response_len` (длина сгенерированного ответа)
    
-   `latency` (задержка)
    
-   `status_flag` (оценка успешности: `Correctly_Rejected`, `Hallucination_Risk` и др.)
    
-   `response` (итоговый текст ответа)
    

**10.3. Формирование «золотого набора» вопросов** Был подготовлен набор из вопросов, покрывающий разные аспекты системы:

-   **Известные темы (ожидается отказ):** Архитектура C4, механизмы Windows Task Scheduler, протокол Protobuf, детали реализации Smart Intercom для PropDevelopment. (то с предыдущего спринта)
    
-   **Отсутствующие темы (ожидается отказ):** Запросы про VoidCore, Synth Flux и Xarn Velgor.
    

**10.4. Автоматическое тестирование** Скрипт-оценщик в автоматическом режиме прогнал «золотой набор» через пайплайн RAG-бота. Алгоритм оценки сверял наличие извлеченного контекста (`has_chunks`) с итоговым ответом модели, выставляя соответствующие флаги успешности. В ходе тестирования были успешно зафиксированы как корректные отказы, так и случаи подмены фактов.

**10.5. Анализ логов** Анализ файла `evaluation_results.jsonl` позволил выявить следующие закономерности:

-   **Темы, по которым бот часто не отвечает:** Система успешно отказывается отвечать (возвращает строго `Я не знаю.` со статусом `Correctly_Rejected`) на узкие запросы вроде «Who is Xarn Velgor?», а также на специфичные технические вопросы (протокол Protobuf), если они не попали в выборку поиска.
    
-   **Где выдаются нерелевантные ответы (галлюцинации):** Главная проблема возникает при пустом индексе (`has_chunks: false`). Если модель считает, что может ответить на вопрос из общих знаний, она игнорирует отсутствие контекста. Это приводит к статусу `Hallucination_Risk`.
    
    -   _Пример 1:_ Запрос про **VoidCore** спровоцировал ответ о «боевой космической станции размером с луну» (влияние научной фантастики на веса модели).
        
    -   _Пример 2:_ Запрос про **Synth Flux** выдал справку о концепциях музыкального продакшена.
        
    -   _Пример 3:_ На запросы по **Windows Task Scheduler** выдаются стандартные общие инструкции из интернета, не привязанные к специфике скриптов.
    
-   **Где нужно расширить базу знаний:** Чтобы перевести статусы из категории `Hallucination_Risk` в `SUCCESS`, в индекс необходимо добавить:
    
    1.  Техническую документацию по требованиям к C4-архитектуре.
        
    2.  Внутренние ТЗ по Smart Intercom и Smart Barrier.
        
    3.  Регламенты настройки Windows Task Scheduler для конкретных скриптов обновления.
        
    4.  Если VoidCore и Synth Flux являются реальными сущностями текущих задач — добавить их лор или техническое описание.

**10.6. Диаграмма**

![Диаграмма последовательности](evaluate.png)

[Открыть исходный файл диаграммы (.mmd)](evaluate.mmd)



## 11. Резюме и рекомендации для QuantumForge Software

**11.1. Какие темы плохо покрыты** Выявлены разрывы в знаниях:

-   Технические спецификации Protobuf для ПО компании.
    
-   Специфические C4-модели архитектуры QuantumForge.
    
**11.2. Статистика «пробелов»**

-   Выявлено 8 критических зон.
    
-   Система требует донастройки порогового фильтра, чтобы исключить галлюцинации при низком `similarity_score`.
    

**11.3. Рекомендации по улучшению**

1.  **Интеграция:** Векторизовать текущие ТЗ и архитектурные диаграммы компании.
    
2.  **Thresholding:** В `RAG_bot.py` реализовать блокировку ответа, если `score` найденных чанков ниже заданного порога.
    
3.  **Hard System Prompt:** Усилить запрет на использование внешних знаний.
    
4.  **Автоматизация:** Включить `evaluate.py` в процесс CI/CD для регрессионного тестирования при обновлении базы.